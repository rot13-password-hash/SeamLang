...
namespace seam::parser
{
	...
	ir::ast::expression_list parser::parse_expression_list()
	{
	...
	std::unique_ptr<ir::ast::call_expression> parser::parse_call_expression(std::unique_ptr<ir::ast::expression> function)
	{
	...
	std::unique_ptr<ir::ast::expression> parser::parse_prefix_expression()
	{
		const auto current_lexeme = lexer_.current_lexeme();
	...
	std::unique_ptr<ir::ast::expression> parser::parse_expression()
	{
		const auto start_position = lexer_.current_lexeme().position;
	...
	std::unique_ptr<ir::ast::return_statement> parser::parse_return_statement()
	{
		const auto start_position = lexer_.current_lexeme().position;
		lexer_.next_lexeme();
		...
		return std::make_unique<ir::ast::return_statement>(utils::position_range{ start_position, lexer_.current_lexeme().position }, std::move(expression));
	}

	std::unique_ptr<ir::ast::block> parser::parse_block_statement()
	{
	...
	std::unique_ptr<ir::ast::function_signature> parser::parse_function_signature()
	{
		const auto start_position = lexer_.current_lexeme().position;
	...
	std::unique_ptr<ir::ast::function_definition> parser::parse_function_definition_statement()
	{
		const auto start_position = lexer_.current_lexeme().position;
	...
	std::unique_ptr<ir::ast::extern_function_definition> parser::parse_extern_function_definition_statement()
	{
		return nullptr;
	}

	std::unique_ptr<ir::ast::type_definition> parser::parse_type_definition_statement()
	{
		const auto start_position = lexer_.current_lexeme().position;
	...
	std::unique_ptr<ir::ast::restricted_statement> parser::parse_restricted_statement()
	{
		const auto& current_lexeme = lexer_.current_lexeme();
	...
	std::unique_ptr<ir::ast::restricted_block> parser::parse_restricted_block_statement(bool is_type_scope)
	{
		const auto start_position = lexer_.current_lexeme().position;
	...
	std::unique_ptr<ir::ast::restricted_block> parser::parse()
	{
		// Get first lexeme.
		lexer_.next_lexeme();
